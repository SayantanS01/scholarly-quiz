  ...
  return (
    <div>
      <header className="mb-12">
        <h1 className="text-4xl font-black text-white mb-2 text-primary font-bold tracking-tighter">ADMIN_OVERVIEW</h1>
        <p className="text-primary font-mono text-xs uppercase tracking-[0.3em]">System status: healthy</p>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="card-panel p-8 panel-border">
          <h3 className="text-xl font-bold text-white mb-6 tracking-tight uppercase">RECENT_NODES</h3>
          <div className="space-y-4">
            {recentUsers.map((user: any) => (
              <div key={user.id} className="flex justify-between items-center p-4 bg-white/5 border border-white/5 hover:border-primary transition-all">
                <div>
...
function StatCard({ title, value, icon, color }: { title: string; value: number; icon: string; color: string }) {
  ...
  return (
    <div className={`p-6 panel-border ${colors[color]} bg-card/50`}>
  ...
  return (
    <div className="flex items-center justify-between p-4 bg-white/5 border border-white/5">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-primary text-[10px] font-black uppercase tracking-widest">{status}</span>
      </div>
    </div>
  )
}
