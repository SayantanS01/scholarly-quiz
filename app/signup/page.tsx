  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6 bg-cyber-black">
      <div className="glass-panel p-10 cyber-border w-full max-w-md relative overflow-hidden">
        <div className="absolute top-0 right-0 p-4 text-[10px] text-neon-cyan font-mono opacity-20">REG_NODE_v1.0</div>
        
        <h2 className="text-4xl font-black mb-2 text-white tracking-tighter neon-text">INIT_REGISTRATION</h2>
        <p className="text-slate-500 font-mono text-xs mb-10 tracking-widest uppercase">Generate new encrypted identity</p>

        <form action={action} className="space-y-6">
          <div>
            <label className="block text-[10px] font-black text-neon-cyan mb-2 uppercase tracking-[0.2em]">FULL_NAME</label>
            <input 
              name="name" 
              type="text" 
              ...
              required
            />
            {state?.errors?.name && <p className="text-neon-magenta text-[10px] mt-1 font-bold">{state.errors.name}</p>}
          </div>

          <div>
            <label className="block text-[10px] font-black text-neon-cyan mb-2 uppercase tracking-[0.2em]">IDENTIFIER_EMAIL</label>
            <input 
              name="email" 
              type="email" 
        ...
        <p className="mt-10 text-center text-slate-600 font-mono text-[10px] tracking-widest uppercase">
          ...
          <Link href="/login" className="text-neon-magenta hover:text-white transition-colors font-black">
            [ INIT_LOGIN ]
          </Link>
        </p>
      </div>
