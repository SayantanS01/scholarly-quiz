  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6 bg-background">
      <div className="card-panel p-10 panel-border w-full max-w-md relative overflow-hidden">
        <div className="absolute top-0 right-0 p-4 text-[10px] text-primary font-mono opacity-20">AUTH_SEC_v4.2</div>
        
        <h2 className="text-4xl font-black mb-2 text-white tracking-tighter text-primary font-bold">INIT_LOGIN</h2>
        ...
        <form action={action} className="space-y-8">
          <div>
            <label className="block text-[10px] font-black text-primary mb-2 uppercase tracking-[0.2em]">IDENTIFIER</label>
            <input 
              name="email" 
              type="email" 
        ...
        <p className="mt-10 text-center text-slate-600 font-mono text-[10px] tracking-widest uppercase">
          No identity found?{' '}
          <Link href="/signup" className="text-primary hover:text-white transition-colors font-black">
            [ REGISTER_NODE ]
          </Link>
        </p>
      </div>
