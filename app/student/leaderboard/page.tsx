  ...
  return (
    <div>
      <header className="mb-12">
        <h1 className="text-4xl font-black text-white mb-2 text-primary font-bold tracking-tighter">RANKING_BOARD</h1>
        <p className="text-primary font-mono text-xs uppercase tracking-[0.3em]">Top Performers in Network</p>
      </header>
      ...
      <div className="card-panel panel-border overflow-hidden">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-0 divide-y md:divide-y-0 md:divide-x divide-white/10 border-b border-white/10">
          {leaderboard.slice(0, 3).map((user: any, i: number) => (
            <div key={user.name} className="p-12 text-center flex flex-col items-center group hover:bg-primary/10 transition-all">
              ...
              <div className="text-primary font-black text-5xl mb-6 text-primary font-bold">{user.avgScore}%</div>
        ...
        <div className="p-8">
          <table className="w-full text-left font-mono">
            <thead>
              <tr className="text-primary text-[10px] font-black tracking-[0.3em] uppercase border-b border-white/5">
            ...
            <tbody className="divide-y divide-white/5">
              {leaderboard.slice(3).map((user: any, i: number) => (
                <tr key={user.name} className="hover:bg-primary/10 transition-colors group">
                  <td className="py-6 px-4 text-slate-500 font-bold tracking-tighter">#{i + 4}</td>
                  <td className="py-6 px-4 font-black text-white group-hover:text-primary transition-colors uppercase">{user.name}</td>
                  <td className="py-6 px-4 text-center">
                    <span className="text-primary font-black">{user.avgScore}%</span>
                  </td>
